...
contract DPexPair is IUniswapV2Pair, Initializable, Governable, DPexERC20, SafeGas {
    ...
    function mint(address to) external override useCHI lock returns (uint liquidity) {
    ...
    // this low-level function should be called from a contract which performs important safety checks
    function burn(address to) external override useCHI lock returns (uint amount0, uint amount1) {
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
    ...
    // this low-level function should be called from a contract which performs important safety checks
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override useCHI lock {
        require(amount0Out > 0 || amount1Out > 0, 'UniswapV2: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'UniswapV2: INSUFFICIENT_LIQUIDITY');
    
        (uint amount0In, uint amount1In) = _update(_reserve0, _reserve1, amount0Out, amount1Out, to, data);
        emit Swap(msg.sender, amount0In, amount1In, amount0Out, amount1Out, to);
    }
    function _update(uint112 _reserve0, uint112 _reserve1, uint amount0Out, uint amount1Out, 
    ...
    function skim(address to) external override useCHI lock {
        address _token0 = token0; // gas savings
        address _token1 = token1; // gas savings
    ...
    function sync() external override useCHI lock {
